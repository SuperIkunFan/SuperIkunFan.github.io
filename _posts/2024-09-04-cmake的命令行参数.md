...
主要目的：获取cmake支持哪些跨平台的操作，例如打印字符串啊，或者查看环境变量等，又有不同平台有不同的命令方式，所以使用cmake就可以达到快平台的效果
`cmake -E SpecificCommand CommandARG`  
使用如上格式就可以调用cmake内置的命令，达到不同平台上相同的终端命令效果。相比较而言，它并不用于直接输入到命令行中，而是写道CMakeLists.txt文件中，它常常和ADD_CUSTOM_COMMAND进行联合调用。如下格式：  
```
ADD_CUSTOM_COMMAND(
    TARGET my-target
...
```
这样可以达到跨平台效果，其中`${CMAKE_COMMAND}`表示当前执行cmake命令，它包含的完整路径的可执行文件
例如：`message("CMAKE_COMMAND = ${CMAKE_COMMAND}")`就可以打印出当前cmake的位置，我的执行结果为`CMAKE_COMMAND = D:/Program Files/bin/cmake.exe`
