...
在很多时候，一个项目被cmake进行管理时，其往往具有同一套的编译参数或者是几套编译参数。cmake官方给出的需求是：

> 1. cmake用户如何与其他人共享项目配置的通用设置。
> 2. 如何在CI/CD中使用相同的配置项从而达到相同的构建效果

然后个人觉得cmake官方并没有说明白为什么需要preset功能。除开开源库，更多的时候一个项目往往就是一个正式版本，若干个测试版本，因而版本间参数并不一样。一个常见的解决方案就是写一个本地化脚本，如shell或者是powershell脚本，然后直接运行这个脚本即可。在这个脚本中，可以将各个版本间的参数通过命令行的参数传递进去，这似乎也能解决不同版本的参数问题。

个人认为CMake preset功能适用于如下场景：

1. 需要发布多配置、跨平台的库时
2. 开发人员维护预设配置，用户只需选择合适的配置项即可，无需手动编写复杂的构建参数。

一个cmakelists.txt中有几十个option以及相应的参数，以及环境参数等，某一个功能所需要的配置参数集写成一个名字，然后就可以编译了。

## 二、读取方式

了解cmake的preset功能，就需要了解以下问题：

1. cmake如何知道是否需要使用preset功能呢？
    - cmake 通过构建、编译、安装等阶段的`cmake --preset presetName`，判断是否需要激活preset功能，如何输入了，那说明需要preset功能，如果没有，那就不需要preset功能。

2. cmake如何知道使用哪一个配置项呢？
    - 首先cmake是根据位于项目根部的两个命名json文件来获取配置选项的。
        1. CMakePresets.json
        2. CMakeUserPresets.json
    - 为什么会有两个配置文件？
        * CMakePresets.json是共有的项目配置文件，这个一般是放入到git或者是svn作为项目文件进行管理。
        * CMakeUserPresets.json一般不作为项目管理文件的，既可以管理项目又可以调试个人编译配置。
    - 值得注意的是，cmake只会识别CMakePresets.json或者是CMakeUserPresets.json。
        * 例如取名为CMakeMyPresets.json，cmake是不会读取其中的配置文件的。
...
1. name 
    + 这是一个必需的字符串，表示预设的机器友好名称。此标识符用于 cmake --preset 选项中。在同一目录下，CMakePresets.json 和 CMakeUserPresets.json 文件合并后，不能存在两个同名的配置预设。但是，配置预设可以与构建预设、测试预设、打包预设或工作流预设同名。

2. hidden
    + 这是一个可选的布尔值，用于指定预设是否应该被隐藏。如果预设被隐藏，它不能在 --preset= 参数中使用，不会在CMake GUI中显示，并且即使是通过继承获得的，也不需要有有效的生成器（generator）或二进制目录（binaryDir）。隐藏的预设旨在作为其他预设通过继承字段（inherits）继承的基础。
...
6. displayName
    + 一个可选字符串，带有预设的人性化名称。可通过cmake --list-presets看到这些描述

7. description
    + 一个可选字符串，对预设进行人性化描述。可通过cmake --list-presets看到这些描述
...
15. cacheVariables
    + 一个可选的缓存变量映射。键是变量名（不能为空字符串），值可以是 null、布尔值（等同于值为 "TRUE" 或 "FALSE" 并且类型为 BOOL）、代表变量值的字符串（支持宏展开），或包含以下字段的对象：
        - **type**: 一个可选的字符串，表示变量的类型。
        - **value**: 一个必需的字符串或布尔值，表示变量的值。布尔值等同于 "TRUE" 或 "FALSE"。此字段支持宏展开。
    + 缓存变量通过 `inherits` 字段继承，预设的变量将是其自身 `cacheVariables` 与其所有父级的 `cacheVariables` 的并集。
    + 如果此并集中多个预设定义了相同的变量，则应用继承的标准规则。将变量设置为 null 将导致它不被设置，即使从其他预设继承了值。
